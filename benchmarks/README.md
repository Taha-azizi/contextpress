...
**Current headline (222 items, 0.6.14 fidelity):** `low` **6.0%** mean token save and **98.4%** weighted fact retention; `medium` **22.7% / 84.8%**; `high` **47.7% / 72.7%**. Details: [`INFO_FIDELITY.md`](INFO_FIDELITY.md). In 0.7.1, the same 888-compression benchmark ran in **39s** (216s before the 0.7.1 hot-path work).
